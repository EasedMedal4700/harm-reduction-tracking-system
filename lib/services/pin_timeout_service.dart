...
class PinTimeoutService {
  ...
  static const int defaultForegroundTimeout = 5;      // Re-enter PIN after 5 min inactive
  static const int defaultBackgroundTimeout = 60;     // Re-enter PIN after 60 min in background
  static const int defaultMaxSessionDuration = 480;   // Auto-lock after 8 hours (0 = disabled)
  ...
  Future<bool> isPinRequired() async {
    ...
    if (maxSession > 0 && sessionStart != null) {
      ...
      if (sessionDuration > maxSessionMs) {
        print('🔐 PIN required: Max session duration exceeded (${maxSession}min)');
        return true;
      }
    }
    ...
    if (backgroundStart != null) {
      ...
      if (backgroundDuration > backgroundTimeoutMs) {
        print('🔐 PIN required: Background timeout exceeded (${backgroundTimeout}min)');
        return true;
      }
    }
    ...
    if (timeSinceUnlock > foregroundTimeoutMs) {
      print('🔐 PIN required: Foreground timeout exceeded (${foregroundTimeout}min)');
      return true;
    }
    
    final remaining = ((foregroundTimeoutMs - timeSinceUnlock) / 1000 / 60).round();
    print('✅ PIN not required. Last unlock: $lastUnlockTime, ${remaining}min remaining');
    return false;
  }
