...
class DecayService {
  ...
  List<CurvePoint> generateDecayCurveForDose({
    ...
    for (double hour = -hoursBack.toDouble(); 
         hour <= hoursForward.toDouble(); 
         hour += stepHours) {
      
      final timePoint = referenceTime.add(Duration(
        minutes: (hour * 60).round(),
      ));
      
      // Calculate hours elapsed from this dose's start time
      final hoursElapsed = timePoint.difference(dose.startTime).inMinutes / 60.0;
  ...
  List<CurvePoint> generateCombinedCurve({
    ...
    final allCurves = doses.map((dose) => 
      generateDecayCurveForDose(
        dose: dose,
        halfLife: halfLife,
    ...
    for (int i = 0; i < numPoints; i++) {
      ...
      combinedPoints.add(CurvePoint(
        hours: hour,
        remainingMg: totalRemaining,
      ));
    }
