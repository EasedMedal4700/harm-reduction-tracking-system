...
class BucketToleranceService {
  ...
  static Future<BucketToleranceModel?> fetchToleranceModel(String substance) async {
    try {
      ...
      final toleranceModelJson = response['tolerance_model'] as Map<String, dynamic>;
  ...
  static Future<Map<String, dynamic>> calculateSubstanceTolerance({
    ...
    try {
      ...
      final bucketResults = BucketToleranceCalculator.calculateSubstanceTolerance(
        model: model,
        useEvents: useEvents,
      ...
      final overallTolerance = BucketToleranceCalculator.calculateOverallSubstanceTolerance(
  ...
  static Future<Map<String, dynamic>> calculateSystemTolerance({
    ...
    try {
      ...
      final substancesResponse = await _supabase
          ...
          .select('substance')
          .eq('user_id', userId)
          .gte('timestamp', DateTime.now().subtract(const Duration(days: 90)).toIso8601String());
      ...
      final allSubstanceResults = <String, Map<String, BucketToleranceResult>>{};
      ...
      for (final substance in uniqueSubstances) {
        ...
        if (result['buckets'] != null) {
          allSubstanceResults[substance] = result['buckets'] as Map<String, BucketToleranceResult>;
        }
      }

      // Calculate system tolerance (cross-tolerance)
      final systemBucketTolerances = BucketToleranceCalculator.calculateSystemTolerance(
        allSubstanceResults: allSubstanceResults,
      );

      final overallSystemTolerance = BucketToleranceCalculator.calculateOverallSystemTolerance(
