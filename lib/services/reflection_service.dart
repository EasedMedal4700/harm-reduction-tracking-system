...
class ReflectionService {
  Future<int> getNextReflectionId() async {
    try {
      ...
      final nextId = response.isNotEmpty ? (response[0]['reflection_id'] as int) + 1 : 1;
      ErrorHandler.logDebug('ReflectionService', 'Next reflection ID: $nextId');
      return nextId;
    } catch (e, stackTrace) {
      ErrorHandler.logError('ReflectionService.getNextReflectionId', e, stackTrace);
  ...
  Future<void> saveReflection(Reflection reflection, List<int> relatedEntries) async {
    try {
      ErrorHandler.logDebug('ReflectionService', 'Saving new reflection with ${relatedEntries.length} related entries');
      ...
      ErrorHandler.logInfo('ReflectionService', 'Reflection saved successfully with ID: $nextId');
    } catch (e, stackTrace) {
  ...
  Future<void> updateReflection(String id, Map<String, dynamic> data) async {
    try {
      ErrorHandler.logDebug('ReflectionService', 'Updating reflection $id with ${data.keys.length} fields');
      ...
      ErrorHandler.logInfo('ReflectionService', 'Reflection updated successfully: $id');
    } catch (e, stackTrace) {
      ErrorHandler.logError('ReflectionService.updateReflection', e, stackTrace);
  ...
  Future<ReflectionModel?> fetchReflectionById(String id) async {
    try {
      ErrorHandler.logDebug('ReflectionService', 'Fetching reflection by ID: $id');
      ...
      ErrorHandler.logDebug('ReflectionService', 'Query params - original_id: $id, parsed_id: $parsedId');
      ...
      if (result == null) {
        ErrorHandler.logWarning('ReflectionService', 'No reflection found with ID: $id');
        throw ReflectionNotFoundException(id);
      }
      ...
      ErrorHandler.logError('ReflectionService.fetchReflectionById', e, stackTrace);
      if (e is ReflectionException) rethrow;
      throw ReflectionFetchException(
