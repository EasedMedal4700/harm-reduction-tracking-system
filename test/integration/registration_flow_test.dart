...
void main() {
  ...
  group(
    ...
    () {
      ...
      test(
        'Successful registration creates both Auth user and database entry',
        () async {
          // Register new user
          ...
          expect(
            loginResult,
            true,
            reason: 'Should be able to login after registration',
          );
          ...
          expect(
            userData['is_admin'],
            false,
            reason: 'New users should not be admin',
          );
          expect(
            userData['user_id'],
            isNotNull,
          ...
          expect(
            userId,
            isA<String>(),
            reason: 'User ID should be a string (UUID)',
          );
          expect(
            userId.isNotEmpty,
            true,
      ...
      test('Registration with duplicate email fails', () async {
        ...
        expect(
          firstResult.success,
          true,
          reason: 'First registration should succeed',
        );
      ...
      test(
        'Registration with empty display name uses email as fallback',
        () async {
          final result = await authService.register(
            email: testEmail,
            password: testPassword,
      ...
      test(
        'Registration with null display name uses email as fallback',
        () async {
          final result = await authService.register(
            email: testEmail,
            password: testPassword,
    ...
    skip: const String.fromEnvironment('SUPABASE_URL').isEmpty
