...
// Generate mocks
@GenerateMocks([SupabaseClient, GoTrueClient, User, EncryptionServiceV2])
...
void main() {
  ...
  setUp(() {
    ...
    when(mockEncryptionService.decryptFields(any, any)).thenAnswer((
      invocation,
    ) {
      return Future.value(
  ...
  group('ActivityService', () {
    test(
      'fetchRecentActivity returns combined and sorted activities',
      () async {
        // Mock data
        final entriesData = [
          {
        ...
        final fakeEntriesQueryBuilder = FakeSupabaseQueryBuilder(
          fakeEntriesBuilder,
        );
        final fakeCravingsQueryBuilder = FakeSupabaseQueryBuilder(
          fakeCravingsBuilder,
        );
        final fakeReflectionsQueryBuilder = FakeSupabaseQueryBuilder(
          fakeReflectionsBuilder,
        );

        // Setup Supabase client to return fake builders
        when(
          mockSupabaseClient.from('drug_use'),
        ).thenAnswer((_) => fakeEntriesQueryBuilder);
        when(
          mockSupabaseClient.from('cravings'),
        ).thenAnswer((_) => fakeCravingsQueryBuilder);
        when(
          mockSupabaseClient.from('reflections'),
        ).thenAnswer((_) => fakeReflectionsQueryBuilder);
    ...
    test('fetchRecentActivity handles empty data', () async {
      ...
      final fakeCravingsBuilder = FakePostgrestBuilder(
        <Map<String, dynamic>>[],
      );
      final fakeReflectionsBuilder = FakePostgrestBuilder(
        <Map<String, dynamic>>[],
      );
    ...
    test(
      'fetchRecentActivity throws StateError when user not logged in',
      () async {
