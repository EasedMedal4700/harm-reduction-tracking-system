...
void main() {
  ...
  group('PinTimeoutService', () {
    ...
    group('Timeout Expiry', () {
      ...
      test('isPinRequired returns true after foreground timeout at 5 minutes', () async {
      ...
      test('short background trip should not require PIN before foreground timeout', () async {
        ...
        final required = await service.isPinRequired();
        expect(required, isFalse, 
            reason: 'Should not require PIN: 4min < 5min foreground timeout, 1min < 60min background timeout');
      });

      test('isPinRequired returns true after background timeout (over 60 min)', () async {
      ...
      test('isPinRequired returns false when background time is under 60 min', () async {
      ...
      test('recordForegroundResume extends session for subsequent checks', () async {
        final now = DateTime.now();
        SharedPreferences.setMockInitialValues({
          'pin_foreground_timeout_minutes': 5,
          'pin_background_timeout_minutes': 60,
