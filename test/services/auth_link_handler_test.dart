...
void main() {
  group('Deep Link URL Parsing', () {
    ...
    test('parses Supabase verification URL with recovery type', () {
      ...
      expect(uri.queryParameters['redirect_to'], 'substancecheck://reset-password');
    });
  ...
  group('Verification Type Detection', () {
    test('identifies email verification types', () {
      ...
      for (final type in emailTypes) {
        ...
        expect(isEmailVerification, isTrue, reason: 'Type "$type" should be email verification');
      }
    });
  ...
  group('Token Extraction', () {
    test('extracts access_token from query parameters', () {
      final uri = Uri.parse('substancecheck://auth?access_token=test_token_123');
  ...
  group('URL Path Detection', () {
    test('detects Supabase auth verify path', () {
      ...
      for (final path in paths) {
        ...
        expect(isVerifyPath, isTrue, reason: 'Path "$path" should be detected as verify');
      }
    });
    ...
    test('non-verify paths are not detected', () {
      ...
      for (final path in paths) {
        final isVerifyPath = path.contains('/auth/v1/verify') || path.contains('/verify');
